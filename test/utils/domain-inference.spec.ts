...
describe('domain-inference (CLI provider path)', () => {
  ...
  it('routes through the CLI chain, resolving domain + components without constructing ChatAnthropic', async () => {
    ...
    const res = await mod.inferDomainAndComponents(makeIssue());
    // Post-#134 union: the CLI path returns a full InferenceResult, so layer defaults
    // to cht-core when the provider omits it (tolerant mapping, same as the API path).
    expect(res).to.deep.equal({ domain: 'messaging', components: ['api/sms'], layer: 'cht-core' });
  ...
  it('short-circuits to ticket values without calling the provider when both domain and components are present', async () => {
    ...
    // Post-#134 union: the short-circuit return also carries layer (defaults to cht-core
    // when the ticket omits it).
    expect(res).to.deep.equal({ domain: 'contacts', components: ['api/x'], layer: 'cht-core' });
