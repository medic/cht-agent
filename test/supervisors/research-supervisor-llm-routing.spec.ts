...
const createContextAnalysis = (overrides: Partial<ContextAnalysisResult> = {}): ContextAnalysisResult => ({
  ...
  // #134 removed historicalSuccessRate from ContextAnalysisResult (its own
  // context-analysis-agent tests now assert the field's absence); this 02c fixture
  // predated that removal, so the stale field is dropped here to match the union type.
  relatedDomains: ['contacts'],
  ...overrides,
});
