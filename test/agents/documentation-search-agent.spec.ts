...
describe('DocumentationSearchAgent', () => {
  ...
  describe('search', () => {
    ...
    it('should return research findings for infrastructure domain', async () => {
      const issue = createTestIssue({
        technical_context: { domain: 'infrastructure', components: [] },
      });
  ...
  describe('identifyRelatedDomains', () => {
    ...
    it('should identify infrastructure domain from topics', () => {
      const topics = ['docker', 'kubernetes'];

      const domains = agent.identifyRelatedDomains(topics);

      expect(domains).to.include('infrastructure');
    });
  ...
  describe('buildResearchFindings (askQuestion consumption)', () => {
    it('should merge answer sources with search-doc references without duplicating URLs', () => {
      const issue = createTestIssue();
      const searchDocs = [
        { title: 'Doc 1', section: 'S1', content: 'contact hierarchy', sourceUrl: 'https://a.com' },
      ];
      const answer = {
        answer: 'You should configure the contact hierarchy carefully.',
        sources: [
          { title: 'Doc 1 dup', url: 'https://a.com' }, // duplicate URL — must not be re-added
          { title: 'Extra source', url: 'https://b.com' }, // new URL — must be added
        ],
      };

      const findings = (agent as any).buildResearchFindings(searchDocs, answer, issue);

      const urls = findings.documentationReferences.map((r: any) => r.url);
      expect(urls).to.include('https://a.com');
      expect(urls).to.include('https://b.com');
      expect(urls.filter((u: string) => u === 'https://a.com')).to.have.lengthOf(1);
      expect(findings.source).to.equal('kapa-ai');
    });
  });
