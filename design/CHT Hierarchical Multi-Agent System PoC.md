...
type CHTCategory =
  // Core Application
  | 'forms' | 'tasks' | 'targets' | 'reports' | 'workflows'
  // System Architecture
  | 'permissions' | 'messaging' | 'sync' | 'purging'
  // Configuration
  | 'app-settings' | 'contact-hierarchy' | 'translations' | 'branding'
  // Technical Components
  | 'api' | 'database' | 'sentinel' | 'webapp'
  // Development
  | 'testing' | 'deployment' | 'performance' | 'migrations'
  // CHT Tools
  | 'cht-conf' | 'cht-datasource' | 'cht-toolbox' | 'test-data';
```

### Category Relationships

Categories are interconnected. When researching `forms`, the system also checks related categories:
- `forms` → `workflows`, `permissions`, `testing`, `reports`
...
# View context state
cht-agent context --task-id <id> [--section research|plan|updates]
```
