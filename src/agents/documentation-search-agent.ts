...
export interface DocumentationSearchAgentOptions {
  /** Model name (reserved for future LLM-assisted parsing) */
  modelName?: string;
  /** Custom MCP client (useful for testing) */
  mcpClient?: MCPClient;
  /** Use mock responses instead of real MCP (for unit tests only) */
  useMockMCP?: boolean;
  /** Override the MCP server URL (defaults to MCP_SERVER_URL env or the built-in default) */
  mcpServerUrl?: string;
}
...
export class DocumentationSearchAgent {
  ...
  constructor(options: DocumentationSearchAgentOptions = {}) {
    ...
    this.mcpClient = options.mcpClient ?? new MCPClient({ serverUrl: this.mcpServerUrl });
