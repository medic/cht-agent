...
// ============================================================================
// P5: contact-form deterministic spec generation
// ============================================================================
//
// cht-conf-test-harness 3.0.15 has NO `loadContactForm` (only fill-based contact
// APIs with brittle answer arrays), so a contact-form spec CANNOT use the harness
// the way the app-form template does. The contact template is therefore a PLAIN
// mocha+chai spec whose oracle is a DIRECT XML read of `forms/contact/<form>.xml`
// with a self-contained inline bind-attribute extractor — no harness, no
// workbench imports (the partner repo cannot import workbench code). It asserts
// the FULL P2 attrs map (value attrs by equality, null attrs by ABSENCE), which
// is what makes an attrs-only fix such as M8's calculate-removal provable.

/** Where the contact-form generated spec lives (repo-relative to the config root). */
const CONTACT_TEST_DIR = FORMS_TEST_DIR; // test/forms — contact specs sit beside app specs

/** The generated contact-form spec: destination + content + overwrite metadata. */
export interface GeneratedContactFormSpec {
  relPath: string;
  content: string;
  /** True when a partner `<form>.spec.js` already sits beside our `.agent.spec.js`. */
  overwriteAvoided: boolean;
}

/**
 * The scenario a contact-form spec asserts, derived deterministically from the
 * descriptor + verified bindDiff: the target bind nodeset and the FULL attrs map
 * (value = must-carry, null = must-be-absent).
 */
export interface ContactSpecScenario {
  form: string;
  nodeset: string;
...
/**
 * Resolve the contact-form spec destination: ALWAYS the `<form>.agent.spec.js`
 * house location (an agent-owned file we may overwrite — it is our prior output,
 * never a hand-authored partner spec). This never touches a partner
 * `<form>.spec.js`, which is the never-overwrite guarantee. `overwriteAvoided`
 * reports whether such a partner spec exists alongside (informational).
 */
export const resolveContactSpecPath = (configRoot: string, form: string): SpecPathResolution => {
  const partnerRel = path.join(CONTACT_TEST_DIR, `${form}.spec.js`);
  return {
    relPath: path.join(CONTACT_TEST_DIR, `${form}.agent.spec.js`),
    overwriteAvoided: fs.existsSync(path.join(configRoot, partnerRel)),
  };
};

/** Derive the contact-form scenario from the descriptor + verified bindDiff. */
export const deriveContactScenario = (
  descriptor: XlsformFixDescriptor,
  bindDiff: XlsformBindDiff
): ContactSpecScenario => ({
  form: descriptor.form,
  nodeset: bindDiff.nodeset,
  attrs: bindDiff.attrs,
  ...(descriptor.rationale ? { rationale: descriptor.rationale } : {}),
});

/**
 * Render the self-contained contact-form spec: a plain mocha+chai file (no
 * harness) that reads the COMPILED `forms/contact/<form>.xml` and asserts the
 * target bind exists and carries every attr in the fix's `attrs` map — value
 * attrs by whitespace-normalized equality (matching the workbench inspector),
 * null attrs by absence. The header states which fix it guards and that it
 * asserts the compiled source (regenerate via `convert-contact-forms` first).
 */
export const renderContactFormSpec = (scenario: ContactSpecScenario): string => {
  const { form, nodeset, attrs } = scenario;
  const cmt = (s: string): string => s.replace(/\*\//g, '* /');
  const rationaleComment = scenario.rationale ? ` *   rationale: ${cmt(scenario.rationale)}\n` : '';
  const valueAttrs = Object.entries(attrs).filter(([, v]) => v !== null).map(([k]) => k);
  const absentAttrs = Object.entries(attrs).filter(([, v]) => v === null).map(([k]) => k);
  const attrsSummary =
    `${valueAttrs.length} value attr(s) [${valueAttrs.join(', ') || 'none'}], ` +
    `${absentAttrs.length} absence assertion(s) [${absentAttrs.join(', ') || 'none'}]`;
  return `const { expect } = require('chai');
const fs = require('node:fs');
const path = require('node:path');

/**
 * Generated by the CHT agent (P5 contact-form XLSForm fix) — do not overwrite by hand.
 * Durable red->green oracle for the ${cmt(form)} contact form fix:
 *   target bind: ${cmt(nodeset)}
 *   ${cmt(attrsSummary)}
${rationaleComment} *
 * The pinned test harness (v3.0.15) has no contact-form loader, so this spec does
 * NOT use it: it asserts the COMPILED source XML directly. It reads
 * forms/contact/${cmt(form)}.xml, so REGENERATE it (convert-contact-forms) before
 * running — the assertions fail against the pre-fix XML and pass against the fixed XML.
 */

const FORM = ${jsString(form)};
const TARGET_NODESET = ${jsString(nodeset)};
// The full per-attribute expectation the fix asserted on the regenerated bind:
// a string value = the compiled bind MUST carry it; null = it MUST be absent.
const EXPECTED_ATTRS = ${JSON.stringify(attrs, null, 2)};

const CONTACT_XML_PATH = path.resolve(__dirname, '..', '..', 'forms', 'contact', FORM + '.xml');

function readContactXml() {
  return fs.readFileSync(CONTACT_XML_PATH, 'utf8');
}

// Return the raw attribute region of <bind nodeset="..."> (attribute-order
// agnostic), or undefined when the bind is not present. Inlined so the spec needs
// no workbench dependency.
function findBindAttrs(xml, nodeset) {
  const bindRe = /<bind\\b([^>]*)\\/?>/g;
  let m;
  while ((m = bindRe.exec(xml)) !== null) {
    const ns = /\\bnodeset\\s*=\\s*"([^"]*)"/.exec(m[1]);
    if (ns && ns[1] === nodeset) {
      return m[1];
    }
  }
...
// undefined = bind not found; null = bind present but attr absent; string = value.
function extractBindAttr(xml, nodeset, attr) {
  const region = findBindAttrs(xml, nodeset);
  if (region === undefined) {
    return undefined;
  }
  const escaped = attr.replace(/[.*+?^\${}()|[\\]\\\\]/g, '\\\\$&');
  const re = new RegExp('\\\\b' + escaped + '\\\\s*=\\\\s*"([^"]*)"');
  const found = re.exec(region);
  return found ? decodeXmlAttr(found[1]) : null;
}
...
describe(${jsString(`${form} — contact-form bind fix (${nodeset})`)}, () => {
  it('has the target bind in the compiled contact form', () => {
    const xml = readContactXml();
    expect(findBindAttrs(xml, TARGET_NODESET), 'target bind ' + TARGET_NODESET + ' not found in ' + CONTACT_XML_PATH)
      .to.not.be.undefined;
  });

  Object.keys(EXPECTED_ATTRS).forEach((attr) => {
    const expected = EXPECTED_ATTRS[attr];
    if (expected === null) {
      it('omits @' + attr + ' on ' + TARGET_NODESET, () => {
        const xml = readContactXml();
        expect(findBindAttrs(xml, TARGET_NODESET), 'target bind not found').to.not.be.undefined;
        expect(
          extractBindAttr(xml, TARGET_NODESET, attr),
          '@' + attr + ' must be absent on ' + TARGET_NODESET,
        ).to.equal(null);
      });
    } else {
      it('sets @' + attr + ' on ' + TARGET_NODESET, () => {
        const xml = readContactXml();
        const actual = extractBindAttr(xml, TARGET_NODESET, attr);
        expect(actual, '@' + attr + ' missing on ' + TARGET_NODESET).to.not.be.oneOf([null, undefined]);
        expect(normalizeWhitespace(actual)).to.equal(normalizeWhitespace(expected));
      });
    }
  });
});
...
/**
 * Generate the deterministic contact-form spec: resolve the agent-owned
 * destination (never overwriting a partner `<form>.spec.js`), derive the scenario
 * from the descriptor/bindDiff, and render the self-contained plain-mocha JS.
 * Pure aside from the single partner-spec existence probe.
 */
export const generateContactFormSpec = (
  descriptor: XlsformFixDescriptor,
  bindDiff: XlsformBindDiff,
  configRoot: string
): GeneratedContactFormSpec => {
  const { relPath, overwriteAvoided } = resolveContactSpecPath(configRoot, descriptor.form);
  const scenario = deriveContactScenario(descriptor, bindDiff);
  const content = renderContactFormSpec(scenario);
  return { relPath, content, overwriteAvoided };
};
