...
Review on #122 grouped this with `10071` and `10180` as pure cht-datasource extension and asked for `data-access` as the primary domain with `secondaryDomains: [forms-and-reports]`. The premise is weaker for this draft than for those two: the bulk of the change is `shared-libs/cht-datasource`, but the diff also carries `api/src/controllers/report.js`, `shared-libs/lineage/src/index.d.ts` and two webapp files (see Solution — the webapp pair is import-syntax fallout from the new type declarations). Either way the re-key is deliberately not made here: neither the enum value nor the `secondaryDomains` field exists in the schema yet, and the recommendation on that same PR is to ship both as one coordinated taxonomy change rather than through a content PR.
