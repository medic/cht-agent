...
- #10224: UI Extensions epic. PR #11057 merged into its `10224-ui-extensions` feature branch, and that branch reached master as the epic's squash, medic/cht-core#11050 (commit `180c29ecf`). `source_sha` here is #11057's own merge commit on the feature branch, so it is not an ancestor of master and will not resolve in a plain clone — look for the change under `180c29ecf`.
