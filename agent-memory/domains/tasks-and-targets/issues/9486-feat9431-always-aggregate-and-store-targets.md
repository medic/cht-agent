...
summary: Targets and tasks were computed on page visits, with only the once-per-session 120s ensure-freshness debounces (one for tasks, one for targets) as a background fallback, and aggregated targets were never persisted. The rules engine now always aggregates and persists targets and recalculates tasks automatically whenever rules state or documents change, with a 1s debounce to batch bursts of changes.
services:
  - webapp
techStack:
  - typescript
...
Target aggregation and task recalculation only produced up-to-date results when the user navigated to certain pages (e.g. the targets/tasks views) or when the once-per-session 120s ensure-freshness debounces fired: a change-feed hook already ran on each incoming document, but all it did was mark the affected contacts dirty — invalidating cached emissions without recomputing them — and aggregated targets were never persisted at all. This left targets and tasks stale until a page visit or the freshness pass, and meant newly created or freshly synced documents did not update targets/tasks promptly.
...
Debounced, change-driven recalculation: subscribe to the CouchDB changes feed via `ChangesService` (key `mark-contacts-dirty`) inside rules-engine.service.ts and debounce (`DEBOUNCE_CHANGE_MILLIS = 1000`) before invoking the rules engine, then aggregate and persist target state. Key files: webapp/src/ts/services/rules-engine.service.ts (change subscription + debounce) and webapp/src/ts/services/db-sync.service.ts (only to make `inProgressSync` awaitable), shared-libs/rules-engine/src/target-state.js and shared-libs/rules-engine/src/rules-state-store.js (aggregate + store targets), shared-libs/rules-engine/src/provider-wireup.js and pouchdb-provider.js (persistence wiring).
