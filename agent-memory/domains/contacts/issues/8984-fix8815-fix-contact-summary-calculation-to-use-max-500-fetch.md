...
summary: The contact-summary calculation only received up to 50 of a contact's reports, so contacts with more than 50 reports produced incomplete summaries (e.g. a missing pregnancy card when the relevant report fell outside the first 50). The fix raises the report fetch limit to 500 so the calculation has full report context.
services:
  - webapp
techStack:
  - typescript
...
Contacts associated with more than 50 reports had their contact-summary computed from only the first 50 reports, so the calculation lacked full report context. Configured cards (e.g. a woman's pregnancy card) failed to appear when the relevant report fell outside the first 50 retrieved reports. The defect had existed at least since version 3.9.

## Root Cause

The report fetch driving the contact view model generator relied on the default search result cap of 50 (defined in search.service.ts), so at most 50 reports were passed into the contact-summary calculation regardless of how many reports the contact actually had.

## Solution

Updated the contact view model generator to request up to 500 reports when loading a contact's reports for the contact-summary calculation, so the calculation receives the full set of associated reports (bounded at the new 500 limit) rather than only the first 50.
...
Added/updated an e2e WebdriverIO spec (tests/e2e/default/contacts/contact-details.wdio-spec.js) asserting that the contact-summary pregnancy card appears even when the relevant pregnancy report is beyond the first 50 reports, backed by a pregnancy report factory (tests/factories/cht/reports/pregnancy.js) to generate the >50-report test scenario.
