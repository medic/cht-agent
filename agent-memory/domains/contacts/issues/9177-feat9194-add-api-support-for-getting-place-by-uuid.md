...
Test coverage is five specs added and seventeen updated. Added: the three new cht-datasource place specs (test/place.spec.ts, test/local/place.spec.ts, test/remote/place.spec.ts), the api place controller spec (api/tests/mocha/controllers/place.spec.js), and the integration spec (tests/integration/api/controllers/place.spec.js). Updated, all seventeen: test/index.spec.ts; the person/login/users api controller specs and the extract-person-contacts migration spec; shared-libs/contacts (people/places); transitions (create_user_for_contacts, death_reporting, registration, update_clinics); user-management users; the integration person spec (tests/integration/api/controllers/person.spec.js); and four webapp karma specs (cht-datasource.service, extract-lineage.service, create-user-for-contacts.transition, user-contact.service). New tests notably cover the not-found error behavior of refactored call sites that previously relied on PouchDB.get throwing but was untested.
