...
There was no clean, tested way to retrieve a place-type contact by UUID through the cht-datasource abstraction or a REST API endpoint — only get-person/get-person-with-lineage existed (from #9065). Across the codebase, place retrieval relied on ad-hoc PouchDB.get calls, frequently depending on its implicit not-found-throwing behavior without explicit handling or tests.
