...
1. In `sendMessage`'s existing `.catch`, test exactly `err?.statusCode === 400` (the added comment cites https://rapidpro.io/api/v2/ — "Do not retry with the same values"). `statusCode` is the property shape at the time of this fix; the same branch on current master tests `status` instead (api/src/services/rapidpro.js:109), and the Testing bullet's `.rejects({ statusCode: 400 })` stub is of the same vintage
2. Return `getStateUpdate(STATUS_MAP.failed, message.id, undefined)` so the message becomes `state: 'failed'`, `details: 'Failed'`, with no gatewayRef — the error body (the `urns` validation detail) is never parsed, only logged verbatim by the pre-existing `logger.error` call ("Error thrown when trying to send message: %o", rapidpro.js:110), which is untouched — the `// ignore error, sending the message will be retried later` comment moved below it, leaving the log ahead of the new branch
3. Once the task state is `failed` it no longer matches the `pending-or-forwarded` key of `medic/messages_by_state` that `getOutgoingMessages()` (in api/src/services/messaging.js) uses, so the outgoing sweep stops re-sending it; and because `STATUS_MAP.failed` carries `final: true`, it also drops out of `NON_FINAL_STATES` so the `gateway_messages_by_state` status poll skips it
